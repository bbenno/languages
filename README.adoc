...
Currently, this gem comes without any translation names for the languages for memory reasons.

Certain translations could be added to an project as follows:

. Download the PO file for the intended translation language from https://salsa.debian.org/iso-codes-team/iso-codes[]
. Download file:bin/i18n[] to convert the `.po` to `.yml` file
